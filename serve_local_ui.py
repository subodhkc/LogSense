# serve_local_ui.py - Local development server for UI testing
import uvicorn
...
app = FastAPI(title="LogSense - AI Log Analysis (Local)", version="1.0.0")
...
async def submit_context(request: Request):
    ...
    try:
        ...
        print(f"[CONTEXT] Saved context: {list(data.keys())}")
        return JSONResponse({"status": "success", "message": "Context saved successfully"})
    except Exception as e:
        print(f"[ERROR] Context save failed: {e}")
...
async def analyze_log(file: UploadFile = File(...)):
    ...
    try:
        print(f"[ANALYZE] Processing file: {file.filename}")
        ...
        print(f"[ANALYZE] Processed {len(events)} events, found {len(analysis_result.get('issues', []))} issues")
...
async def generate_report(request: Request):
    ...
    try:
        ...
        user_context_data = _analysis_cache.get('user_context', {})
        ...
        if report_type == 'standard':
            ...
            report_content = {
                ...
                "python_insights": generate_python_insights(events)
            }
        else:
        ...
        print(f"[REPORT] Report generated successfully: {report_type}")
...
def generate_python_insights(events):
    ...
    try:
        ...
        errors = [e for e in events if e.get('level', '').upper() in ['ERROR', 'CRITICAL']]
        warnings = [e for e in events if e.get('level', '').upper() == 'WARNING']
        ...
        components = Counter([e.get('filename', 'Unknown') for e in errors])
        if components:
            top_component = components.most_common(1)[0]
            insights.append(f"Top error-prone file: {top_component[0]} ({top_component[1]} errors)")
...
if __name__ == "__main__":
    print("Starting LogSense Local UI Server...")
